...
const input: string = readPuzzleInput(10);
// input = `
...
while (hasGottenSmaller) {
  ...
  moveStars(stars);
  const drawingArea = calcDrawingArea(stars);
  const area = calcRectangleArea(drawingArea);
...
function createStar(line: string): Star {
  const regex: RegExp = /position=<\s*(-?[0-9]+),\s*(-?[0-9]+)> velocity=<\s*(-?[0-9]+),\s*(-?[0-9]+)>/g;
...
function printStars(stars: Star[], area: Rectangle): boolean {
  const lines: { [y: number]: { [x: number]: string } } = {};
...
function moveStars(stars: Star[]) {
  for (let i = 0; i < stars.length; i++) {
    const { x, y } = stars[i].pos;
    const { x: dx, y: dy } = stars[i].velo;
