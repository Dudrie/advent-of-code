...
const input: string[] = getLinesOfInput(readPuzzleInput(13));
const field: string[][] = [];
const carts: Cart[] = [];
...
function adjustCartDirection(cart: Cart, currTile: string) {
  if (currTile === '+') {
    ...
    const nextTurn: Turn = TURNS[cart.lastTurn];
...
function turnInCurveDirection(cart: Cart, field: string[][]) {
  const { NORTH, EAST, SOUTH, WEST } = Direction;
  const curve: string = field[cart.y][cart.x];
...
function printField(field: string[][], carts: Cart[]) {
  ...
  carts.forEach((cart, idx) => {
    ...
    const c = carts.find((val, i) => i !== idx && compareCarts(cart, val) === 0);
    if (c) {
      cartChar = 'X';
    } else {
      switch (cart.dir) {
