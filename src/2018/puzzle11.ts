/** Puzzle input */
const serialNr: number = 9110;
const grid: number[][] = new Array(301);
...
console.log(
  `Part A -- Top left coordinate: ${maxSqaure.x},${maxSqaure.y} (Power: ${maxSqaure.power})`
);

// Part B
console.log('\nPart B');
const powerSquares: { [id: string]: { [sqSize: number]: number } } = {};
...
const maxForId: { [id: string]: { size: number; power: number } } = {};

for (const id in powerSquares) {
  maxForId[id] = { size: 0, power: Number.MIN_SAFE_INTEGER };

  for (const [size, power] of Object.entries(powerSquares[id])) {
...
let maxDynamicSquare: { id: string; size: number; power: number } = {
  id: '',
  size: 0,
  power: Number.MIN_SAFE_INTEGER,
};
for (const [id, sqInfo] of Object.entries(maxForId)) {
...
console.log(
  `Part B -- Top left coordinate: ${maxDynamicSquare.id},${maxDynamicSquare.size} (Power: ${maxDynamicSquare.power})`
);
