...
let claims: string[] = getLinesOfInput(readPuzzleInput(3));
// claims = [
//     '#1 @ 1,3: 4x4',
//     '#2 @ 3,1: 4x4',
//     '#3 @ 5,5: 2x2'
...
let recs: Rectangle[] = claims.map((input) => createRectangleFromInput(input));
...
function createRectangleFromInput(input: string): Rectangle {
    ...
    let id: number = Number.parseInt(parts[0].substr(1));
    let posArg: string[] = parts[2].substr(0, parts[2].length - 1).split(',');
    let pos: Point = {
        x: Number.parseInt(posArg[0]),
        y: Number.parseInt(posArg[1])
    };

    let dimArgs: string[] = parts[3].split('x');
    let width: number = Number.parseInt(dimArgs[0]);
    let height: number = Number.parseInt(dimArgs[1]);
...
function isOverlapping(recOne: Rectangle, recTwo: Rectangle): boolean {
    return recOne.pos.x < recTwo.pos.x + recTwo.width &&
