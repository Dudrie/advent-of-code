...
let boxIds: string[] = getLinesOfInput(readPuzzleInput(2));
...
for (let i = 0; i < boxIds.length; i++) {
    for (let k = i + 1; k < boxIds.length; k++) {
        let levDist: number = calcLevenshteinDistance(boxIds[i], boxIds[k]);
...
for (let idPair of pairs) {
    ...
    if (wrongCharIdx !== -1) {
        innerRes = idPair.first.substring(0, wrongCharIdx).concat(idPair.first.substring(wrongCharIdx + 1));
    }
...
function calcLevenshteinDistance(idOne: string, idTwo: string): number {
    let dist: number[][] = Array.from({ length: idOne.length + 1 }, () => {
