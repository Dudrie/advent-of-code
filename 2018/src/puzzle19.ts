...
import { Register, Operation, addr, addi, mulr, muli, banr, bani, borr, bori, setr, seti, gtir, gtri, gtrr, eqir, eqri, eqrr } from './puzzle16';

function runLine(line: string) {
    let lineParts: string[] = line.split(' ');
    let opName: string = lineParts.splice(0, 1)[0];
    let nums: number[] = lineParts.map((v) => Number.parseInt(v));

    operations[opName](register, nums[0], nums[1], nums[2]);
}

// #region RUNNING SECTION
let inputData: string[] = getLinesOfInput(readPuzzleInput(19, 0));

let register: Register = [0, 0, 0, 0, 0, 0];
let ipParsed: number = Number.parseInt(/([0-5])/.exec(inputData.splice(0, 1)[0])![0]);
let ipRegister: number = ipParsed;

let operations: { [id: string]: Operation } = {
    addr,
    addi,
    mulr,
    muli,
