...
let input: string[] = getLinesOfInput(readPuzzleInput(6));
...
let topLeft: Point = { x: Number.MAX_SAFE_INTEGER, y: Number.MAX_SAFE_INTEGER };
let bottomRight: Point = { x: Number.MIN_SAFE_INTEGER, y: Number.MIN_SAFE_INTEGER };
...
let areaOfPoint: { [ptString: string]: number } = {};

for (let x = topLeft.x; x < bottomRight.x + 1; x++) {
    for (let y = topLeft.y; y < bottomRight.y + 1; y++) {
        let { pt: ptWithMinDist, isTied } = getPointWithMinDistance({ x, y }, points);

        if (!isTied) {
            areaOfPoint[pointToString(ptWithMinDist)] = (areaOfPoint[pointToString(ptWithMinDist)] || 0) + 1;
        }
    }
}

let offset: number = 3635;
...
pointsToTest.forEach((pt) => {
    let { pt: ptWithMinDist } = getPointWithMinDistance(pt, points);

    areaOfPoint[pointToString(ptWithMinDist)] = Number.POSITIVE_INFINITY;
});

let largestArea: number = 0;
for (let area of Object.values(areaOfPoint)) {
...
console.log(`Part B -- Size of region containg all points with sumDist less ${maxDistance}: ${pointsInArea}`);

// ========== HELPERS ==========
type Point = { x: number, y: number };

function parseLineOfInput(line: string): Point {
    let result: string[] = line.split(',').map((s) => s.trim());
...
function getPointWithMinDistance(pointToCheck: Point, allPoints: Point[]): { pt: Point, isTied: boolean } {
    let { x, y } = pointToCheck;
    let curPoint: Point = { x, y };
    ...
    for (let i = 1; i < points.length; i++) {
        let dist: number = calcManhattanDistance(points[i], curPoint);
