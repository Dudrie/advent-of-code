...
while (recipes.length < input + 10) {
    let { firstElf: first, secondElf: second } = generateRecipes(recipes, firstElf, secondElf);

    firstElf = first;
    secondElf = second;
}

let tenRecipes: number[] = recipes.slice(input, input + 10);
...
let inputLength: number = input.toString().length;
let wantedRecipes: number[] = input.toString().split('').map((n) => Number.parseInt(n));
...
while (!wasFound) {
    ...
    let oldCount: number = recipes.length;
    let { firstElf: first, secondElf: second } = generateRecipes(recipes, firstElf, secondElf);
...
// ========= HELPERS ==========
function generateRecipes(recipes: number[], firstElf: number, secondElf: number): { firstElf: number, secondElf: number } {
    // We do not have enough recipes yet, so create new ones
    let newRecipes: number = recipes[firstElf] + recipes[secondElf];
