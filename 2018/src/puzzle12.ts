...
let input: string[] = getLinesOfInput(readPuzzleInput(12));

type Patterns = { [pattern: string]: string };
let patterns: Patterns = {};
input.forEach((line) => {
    let [pattern, result]: string[] = line.split('=>').map((i) => i.trim());
...
for (let gen = 1; gen <= 20; gen++) {
    let { newGenPots, smallest, greatest } = advancePotsOneGeneration(pots, patterns, smallestIdxWithPot, greatestIdxWithPot);
...
for (let [idx, val] of Object.entries(pots)) {
    if (val === '#') {
...
for (let gen = 1; gen < 50000000000; gen++) {
    ...
    let sum = getPotsSum(pots);
    let added = sum - lastSum;
    lastSum = sum;
    ...
    if (countSameAdded === 100) {
        console.log(`Added the same amount (${added}) a few times -- Assuming it stays equal from now on.`);
        generationIdxEqual = gen;
        break;
    }
}

let totalSum: number = lastSum + (50000000000 - generationIdxEqual) * lastAdded;
...
function getPotsSum(pots: Pots): number {
    let sum = 0;
    for (let [idx, content] of Object.entries(pots)) {
        if (content === '#') {
...
function advancePotsOneGeneration(pots: Pots, patterns: Patterns, smallestIdx: number, greatestIdx: number): { newGenPots: Pots, smallest: number, greatest: number } {
    let newGenPots: Pots = {};
