...
function parseInputLine(line: string) {
    let coordData: string[] = line.split(',').map((c) => c.trim());
    let coords = { startX: 0, endX: 0, startY: 0, endY: 0 };

    for (let data of coordData) {
        let id: string = data.split('=')[0];
        let nums: number[] = data.split('=')[1].split('..').map((v) => Number.parseInt(v));
...
function simulateWater(curPos: Position) {
    let { x, y } = curPos;
    let { SAND, WATER_FLOWING } = TileType;
    ...
    if (isInBoundingBox({ x: curX, y: curY }) && isCorner({ x: curX, y: curY })) {
        let posLeft: Position | undefined = getSideWall({ x: curX, y: curY }, -1);
        let posRight: Position | undefined = getSideWall({ x: curX, y: curY }, 1);
...
function isCorner({ x, y }: Position): boolean {
    return (isTileAtNotWaterable(x, y + 1) && (isTileAtNotWaterable(x - 1, y) || isTileAtNotWaterable(x + 1, y)));
}
...
function getSideWall({ x, y }: Position, inc: number): Position | undefined {
    ...
    while (tile.tileType !== TileType.CLAY && isTileAtNotWaterable(curX, y + 1) && isInBoundingBox({ x: curX, y })) {
        curX += inc;
...
let input: string[] = getLinesOfInput(readPuzzleInput(17));
