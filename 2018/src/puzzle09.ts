...
// Get the game data
let [playerCount, lastMarble] = input.match(/\d+/g)!.map(Number);
...
function getHighScore(scores: Score): number {
    ...
    for (let plyId in scores) {
        let plyScore = scores[plyId];
...
function addToList(value: number, marble: Marble): Marble {
    let toAdd: Marble = { value, next: marble.next, prev: marble };
