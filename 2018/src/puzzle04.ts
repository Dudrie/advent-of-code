...
let lines: string[] = getLinesOfInput(readPuzzleInput(4));
...
let guardsById: { [id: number]: Guard } = {};
...
for (let logEntry of logEntries) {
    if (logEntry.action.indexOf(START_SHIFT) !== -1) {
        ...
        let result: string[] | null = regex.exec(logEntry.action);
        ...
        let guard: Guard = guardsById[guardIdOnShift] || { id: guardIdOnShift, sleepTimeByDay: {}, totalSleepTime: 0 };
...
let guards: Guard[] = Object.values(guardsById);
guards.sort((a, b) => b.totalSleepTime - a.totalSleepTime); // DESC

let mostTiredGuard: Guard = guards[0];
let sleepMinutes: { [id: string]: { [min: number]: number } } = {};

// We're tracking the minutes where a guard sleeps for every guard so we can use this in B.
for (let g of guards) {
    let sleepyTime: { [min: number]: number } = {};

    for (let sleepTimeThatDay of Object.values(g.sleepTimeByDay)) {
        for (let min of sleepTimeThatDay) {
...
let mostTiredSleepMinutes: { [min: number]: number } = sleepMinutes[mostTiredGuard.id];

let minuteMostAsleep: number = -1;
for (let key of Object.keys(mostTiredSleepMinutes)) {
    let currentMinute: number = Number.parseInt(key);
    let currentMaxTime: number = mostTiredSleepMinutes[minuteMostAsleep] || 0;
...
console.log(`PART A -- Answer is: ${mostTiredGuard.id} * ${minuteMostAsleep} = ${mostTiredGuard.id * minuteMostAsleep}`);

// Part B
let mostInOneMinuteId: number = -1;
let currentMax: { minute: number, times: number } = { minute: -1, times: -1 };
for (let key in sleepMinutes) {
    let id: number = Number.parseInt(key);
    let timesPerMinute: [string, number][] = Object.entries(sleepMinutes[id]).sort((a, b) => b[1] - a[1]); // DESC
...
console.log(`Part B -- Answer is: ${mostInOneMinuteId} * ${currentMax.minute} = ${mostInOneMinuteId * currentMax.minute}`);
...
function parseLogEntry(entryString: string): LogEntry {
    ...
    let result: string[] | null = regex.exec(entryString);

    if (!result) {
        throw new Error('ERROR -- NOT WORKING! CALL SANTA IMMEDIATLY!');
    }

    let month: number = Number.parseInt(result[1]);
    let day: number = Number.parseInt(result[2]);
    let minute: number = Number.parseInt(result[3]);
    let action: string = result[4];
...
function determineSleepTime(guard: Guard, startMin: number, endMin: number, day: number) {
    let sleepingTimesThisDay: number[] = guard.sleepTimeByDay[day] || [];
